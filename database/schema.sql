...
CREATE TABLE IF NOT EXISTS `streams` (
    ...
    `bitrate` INT UNSIGNED NULL COMMENT 'kbps',
    `resolution` VARCHAR(20) NULL COMMENT 'e.g. 1920x1080',
    `codec_video` VARCHAR(20) NULL COMMENT 'e.g. H264',
    `codec_audio` VARCHAR(20) NULL COMMENT 'e.g. aac',
    `fps` DECIMAL(5,2) NULL,
    `current_clients` INT UNSIGNED NOT NULL DEFAULT 0,
    `started_at` DATETIME NULL COMMENT 'When stream was started',
    `last_check_at` DATETIME NULL COMMENT 'Last health check',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
